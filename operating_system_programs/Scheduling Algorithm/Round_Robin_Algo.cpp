...
int main(){
	...
	for(int i=0;i<tp;i++){
		...
		cin>>pro[i].at;ind[i].at=pro[i].at;ind[i].in=i;
		cout<<"ENTER BRUST TIME OF PROCESS "<<i+1<<" \n";
		cin>>pro[i].bt;tbt=tbt+pro[i].bt;pro[i].bt1=pro[i].bt;
	}
	for(int i=0;i<tp;i++){
		for(int j=0;j<tp-1;j++){
			if(ind[j].at>ind[j+1].at){
				struct iii at=ind[j];ind[j]=ind[j+1];ind[j+1]=at;
			}
		}
	}
	int cti=2,j=0,k=0,l=0,u=0,arr[tbt];cout<<"\n QUEUE ::: ";
	while(j<tbt){
		
		for(int i=0;i<tp;i++){
			if(ind[i].at<=cti && pro[ind[i].in].ch==0){
